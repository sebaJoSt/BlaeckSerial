...
// "Host" in these comments means the program on the other end of the port, which reads
...
//
// An override has to reach both your sketch and BlaeckSerial.cpp. Several of these size
// members of the class, and if the two files see different values, memory is corrupted
// without any error.
//
//   PlatformIO:   build_flags = -DBLAECK_COMMAND_MAX_CHARS_DEFAULT=128
//   Arduino IDE:  a BlaeckSerialConfig.h in the sketch folder is not found without extra
//                 setup. docs/configuration.md lists the ways to do it.
...
// never ends on a full USB packet (see _bufSend()). 64 covers both full and high speed.
...
// Paused writes
// -------------
// BLAECK.PAUSE_WRITES,<ms> holds back every frame for that long, so a host can close the
// port while the board is quiet. On native-USB boards such as the Giga R1, closing the port
...
// Stored as a byte because every signal and state channel entry holds one.
...
//
// Defined before BlaeckSerial so that editors can offer its methods on the chain.
...
// The records behind each table. Declared here, before BlaeckSerial, because the handle
...
protected:
  ...
  // line, like the two below, because BlaeckSerial is incomplete here.
...
// Defined here because they use BlaeckSerial's private members. Documented at the
...
// Defined here because they need BlaeckSerial to be complete.
