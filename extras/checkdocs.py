...
def extract(tu, path):
    """Every example in the header, as one sketch a compiler can check.

    An example that does not compile is worse than none: it is read as
    authoritative and followed. Rust checks its doc examples by building them,
    which is what lets it require one everywhere; this is the same trick for a
    header. Each example becomes a function body, so it has to be complete
    statements rather than a fragment of a chain - which is a constraint on the
    examples, and the right one.
    """
    seen = set()
    out = ['// GENERATED by extras/checkdocs.py --extract - do not edit.',
           '// Every example from the header, compiled so none of them can rot.',
           '//',
           '// The names an example uses without declaring them live in preamble.h.',
           '#include "preamble.h"',
           '']
    n = 0
    for c in public_api(tu, path):
